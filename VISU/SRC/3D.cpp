...
static float minc, maxc, mingx,maxgx, mingy, maxgy, mingz, maxgz;
...
int edge[MAXSPHERE];
int typs[MAXSPHERE];
bool typf[MAXSPHERE]; // VARIABLE LIST_P[I] AJOUT (22/05/2019)
...
float CONC[MAXSPHERE],GCX[MAXSPHERE],GCY[MAXSPHERE],GCZ[MAXSPHERE];
int PH[MAXSPHERE];
...
}else if(ntype==6){ //Ahmed Ammar AJOUT (22/05/2019)
 // Draw sphere 
			...
			float rays;
					...
					for (i=1;i<=nbsphere;i++ )
					{
					   if(edge[i]>=0){	
						...
						if(ltype==1)
						{
							vali=invmaxv*(def11max-DEF11[i]);
						}		
						else if(ltype==2)
						{
							vali=invmaxv*(def22max-DEF22[i]);
						}   		
						else if(ltype==3)
						{
							vali=invmaxv*(def33max-DEF33[i]);
						} 
						else if(ltype==4)
						{
							vali=invmaxv*(defe11max-DEFE11[i]);
						}		
						else if(ltype==5)
						{
							vali=invmaxv*(defe22max-DEFE22[i]);
						}   		
						else if(ltype>=6)
						{
							vali=invmaxv*(defe33max-DEFE33[i]);
						} 


								  
						  GLfloat Colorval[] = {1.-vali,0.,vali,1.};        
						  glColor4fv(Colorval); 
	...
	glDisable (GL_LIGHT0);	
	 ...
	 invmaxv = ((maxt-mint)<=1.e-12)?0.:(1./(maxt-mint)) ;
	  ...
	  for (i=1;i<=nbsphere;i++ )
	  {
			...
			if(ltype==1)
			{
			vali=invmaxv*(maxt-TEMP[i]);
			}		
			else if(ltype==2)
			{
			vali=invmaxv*(maxfx-FLX[i]);
			}   		
			else if(ltype==3)
			{
			vali=invmaxv*(maxfy-FLY[i]);
			}   
			else if(ltype>=4)
			{
			vali=invmaxv*(maxfz-FLZ[i]);
			}   
...
///////////////
  ...
  if(ltype==0){
	...
	glDisable (GL_LIGHT0);	
	 ...
	 if(ltype==1) {
	 invmaxv = ((maxc-minc)<=1.e-12)?0.:(1./(maxc-minc)) ;
	 }
	 else if(ltype==2) {
         invmaxv = ((maxgx-mingx)<=1.e-12)?0.:(1./(maxgx-mingx)) ;
         }
	 else if(ltype==3) {
         invmaxv = ((maxgy-mingy)<=1.e-12)?0.:(1./(maxgy-mingy)) ;
         }
	 else if(ltype>=4) {
         invmaxv = ((maxgz-mingz)<=1.e-12)?0.:(1./(maxgz-mingz)) ;
         }
	  ...
	  for (i=1;i<=nbsphere;i++ )
	  {
			...
			if(ltype==1)
			{
			vali=invmaxv*(maxc-CONC[i]);
			}		
			else if(ltype==2)
			{
			vali=invmaxv*(maxgx-GCX[i]);
			}   		
			else if(ltype==3)
			{
			vali=invmaxv*(maxgy-GCY[i]);
			}   
			else if(ltype>=4)
			{
			vali=invmaxv*(maxgz-GCZ[i]);
			}   
...
/* Première partie : Creer et remplir le fichier */
  ...
  cout<<"VISU phase      num = 6  &  NOM fichier = vphase "<<endl; 
  cout<<"VISU conc       num = 7  &  NOM fichier = vconw "<<endl; 
 ...
 // if (!g.eof()) 
  {
   ...
   //  cout<<nbsphere<<" "<<nbparoi<<" "<<tps<<endl;
     if(incr==1){
	...
	qcmz[i]=qcmz[i]*X_TOT/X_TOT2;
     ...
     if(incr==1){
	Nt=int(200.*pow(NCONT,-1./4));
	Np=int(200.*pow(NCONT,-1./4));
     }
	...
	Np=int(200.*pow(nbsphere,-1./4));
     ...
     for (i=1;i<=nbsphere;i++)
     {
     fscanf(P_FICHIER, "%i %f %f %f %f %f %f %f %f %f %f \n", &edge[i], &xc[i], &yc[i], &zc[i], &ray[i], &DEF11[i], &DEF22[i], &DEF33[i], &DEFE11[i], &DEFE22[i], &DEFE33[i]);
      xc[i]=xc[i]*X_TOT/X_TOT2;
      yc[i]=yc[i]*X_TOT/X_TOT2;
      zc[i]=zc[i]*X_TOT/X_TOT2;
      ray[i]=ray[i]*X_TOT/X_TOT2;             
	...
	Np=int(200.*pow(nbsphere,-1./4));
     ...
     for (i=1;i<=nbsphere;i++)
     {
      fscanf(P_FICHIER, "%i %f %f %f %f %f %f %f %f %i\n", &edge[i], &xc[i], &yc[i], &zc[i], &ray[i], &TEMP[i], &FLX[i], &FLY[i], &FLZ[i], &PH[i]);
     ...
     // AJOUT VPHASE (22/05/2019)
     else if (ntype==6){
	 fscanf(P_FICHIER, "%i %i %f \n", &nbsphere, &nbparoi, &tps);

     for (i=1;i<=nbsphere;i++)
     {
      fscanf(P_FICHIER, "%i %f %f %f %f %i\n", &edge[i], &xc[i], &yc[i], &zc[i], &ray[i], &typf[i]);
      xc[i]=xc[i]*X_TOT/X_TOT2;
      yc[i]=yc[i]*X_TOT/X_TOT2;
      zc[i]=zc[i]*X_TOT/X_TOT2;  
          ray[i]=ray[i]*X_TOT/X_TOT2;        
	...
	Np=int(200.*pow(nbsphere,-1./4));
     ...
     fscanf(P_FICHIER, "%f %f %f %f %f %f %f %f\n", &minc, &maxc, &mingx, &maxgx, &mingy, &maxgy, &mingz, &maxgz);

     for (i=1;i<=nbsphere;i++)
     {
      fscanf(P_FICHIER, "%i %f %f %f %f %f %f %f %f %i\n", &edge[i], &xc[i], &yc[i], &zc[i], &ray[i], &CONC[i], &GCX[i], &GCY[i], &GCZ[i], &PH[i]);
