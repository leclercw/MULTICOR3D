...
if(boold){
float ttcpu1=(((float) clock())/CLOCKS_PER_SEC);
	// Draw sphere 
		...
		int nump3,numo3,cnump3;
			...
			for (i=1;i<=nbsphere;i++)
			{
			xxi=xc[i]*X_TOT/X_TOT2-X_TOT/2.;
			yyi=yc[i]*X_TOT/X_TOT2-Y_TOT/2.;
			zzi=zc[i]*X_TOT/X_TOT2-Z_TOT/2.;
			rays=ray[i]*X_TOT/X_TOT2;
...
float ttcpu2=(((float) clock())/CLOCKS_PER_SEC)-ttcpu1;
cout<<"tcpu_subdraw:"<<ttcpu2<<endl;
...
float ttcpu1=(((float) clock())/CLOCKS_PER_SEC);
	...
	size_t nOffset =((2*Np+2)*Nt)*nbsphere*sizeof(GLfloat)*3;
	...
	glDrawArrays(GL_TRIANGLE_STRIP,0,((2*Np+2)*Nt)*nbsphere);
...
float ttcpu2=(((float) clock())/CLOCKS_PER_SEC)-ttcpu1;
cout<<"tcpu_subdraw2:"<<ttcpu2<<endl;
