...
inline void ExpMeshHAMZA(int NB_SPH, R* LIST_X, R* LIST_Y, R* LIST_Z, R* LIST_R){
...
// RAYON DES SPHERES
string str_R_SPH = argv[3];
R R_SPH;
from_string<R>(R_SPH,str_R_SPH,dec);

int NA_SPH=floor((H_TOT-2*R_SPH)*sqrt(3)/(4*R_SPH));
R_SPH=H_TOT*sqrt(3)/(4*NA_SPH+2*sqrt(3));
rS=R_SPH;


//R DABx=2*R_SPH*(1-RC);
R DABx=4*R_SPH/sqrt(3);
R DABy=4*R_SPH/sqrt(3);
R DABz=4*R_SPH/sqrt(3);

int pas=max((H_TOT/DABy)+1,(H_TOT/DABx)+1);

//EVALUATION DU NOMBRE DE SPHERES
int NB_SPH=0;

for(int kte=0;kte<pas+1;kte++){    
  for(int jte=0;jte<pas+1;jte++){  
    for(int ite=0;ite<pas+1;ite++){
      
    R zz=R_SPH+kte*DABz;   
    R yy=R_SPH+jte*DABy; 
    R xx=R_SPH+ite*DABx;
	
    if((xx>=-1e-4)&&(xx<=H_TOT+1e-4)&&(yy>=-1e-4)&&(yy<=H_TOT+1e-4)&&(zz>=-1e-4)&&(zz<=H_TOT+1e-4)){NB_SPH++;}
//    cout<<zz<<endl;
    }
  }
}
...
for(int kte=0;kte<pas+1;kte++){    
  for(int jte=0;jte<pas+1;jte++){  
    ...
    R zz=R_SPH+kte*DABz+2*R_SPH/sqrt(3);   
    R yy=R_SPH+jte*DABy+2*R_SPH/sqrt(3); 
    R xx=R_SPH+ite*DABx+2*R_SPH/sqrt(3);
...
for(int kte=0;kte<pas+1;kte++){    
  for(int jte=0;jte<pas+1;jte++){  
    ...
    if((xx>=-1e-4)&&(xx<=H_TOT+1e-4)&&(yy>=-1e-4)&&(yy<=H_TOT+1e-4)&&(zz>=-1e-4)&&(zz<=H_TOT+1e-4)){LIST_X[NB_SPH]=xx;LIST_Y[NB_SPH]=yy;LIST_Z[NB_SPH]=zz;LIST_R[NB_SPH]=R_SPH;NB_SPH++;}
//    cout<<zz<<endl;
    }
  }
}
...
ExpMeshHAMZA(NB_SPH,LIST_X,LIST_Y,LIST_Z,LIST_R);
