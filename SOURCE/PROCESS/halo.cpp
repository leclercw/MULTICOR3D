...
void explore_halo(int iglo, unsigned int ** NOCONT, int * NBCONTCO, int ** CONT, int * NOLOC, bool * PHALO)
{
...
/*
		for(int ih=0;ih<nbh;ih++){
		unsigned int numi=NOHALO[*i][ih];

			int nb=0;

			for(int it=0;it<NBHALO[numi];it++){
			unsigned int numj=NOHALO[numi][it];
                            int na=0;
